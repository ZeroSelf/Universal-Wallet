...
function defineUnwritablePropertyIfPossible(o: any, p: string, value: any) {
  ...
  if (!descriptor || descriptor.writable) {
    ...
    console.warn(
      `Failed to inject ${p} from universal. Probably, other wallet is trying to intercept Universal Wallet`
    );
  }
}
...
const providerProxy = new Proxy(provider, {
  ...
  get: (target, prop) => {
    ...
    if ((typeof prop === 'string' && prop.startsWith('_')) || prop === requestMethodKey) {
      console.warn(
        `[Universal] Attempted access to private method: ${String(prop)} is not allowed for security reasons`
      );
      return undefined;
    }
    return target[prop];
...
defineUnwritablePropertyIfPossible(window, 'universal', providerProxy);

// Many wallets occupy the window.universal namespace, so we need to use a different namespace to avoid conflicts.
defineUnwritablePropertyIfPossible(window, 'universal_wallet', providerProxy);

window.dispatchEvent(new Event('universal#initialized'));
